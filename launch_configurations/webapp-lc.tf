...
resource "aws_launch_configuration" "webapp_lcc" {
  lifecycle { create_before_destroy = true }
  image_id = "${lookup(var.amis, var.region)}"
  instance_type = "${var.instance_type}"
  security_groups = [
...
output "webapp_lcc_id1" {
  value = "${aws_launch_configuration.webapp_lcc.id}"
}
output "webapp_lcc_name1" {
  value = "${aws_launch_configuration.webapp_lcc.name}"
}
